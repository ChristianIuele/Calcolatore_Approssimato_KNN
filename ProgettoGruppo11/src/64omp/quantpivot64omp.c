...
void fit(params* input) {
    ...
    // Usiamo schedule(static) perché il costo è identico per ogni riga.
    ...
    // Il calcolo della distanza è indipendente per ogni cella della matrice indice.
    // collapse(2) unisce i due cicli in uno unico grande parallelo per bilanciare meglio se N è piccolo.
...
void predict(params* input) {
    ...
    // NOTA: Allocazioni spostate DENTRO il ciclo parallelo per thread-safety

    printf("[DEBUG] Allocati knn_ids e knn_dists: k=%d, size_ids=%zu, size_dists=%zu\n",
       input->k, input->k * sizeof(int), input->k * sizeof(type));
        
    // PARALLELIZZAZIONE SU QUERY
    // schedule(dynamic): Fondamentale qui perché il pruning rende le iterazioni disuguali.
    #pragma omp parallel for schedule(dynamic)
    for (int qi = 0; qi < input->nq; qi++) {

        // Alloca buffer riusabili (Privati per ogni thread)
        ...
        if (!input->silent && ((qi + 1) % 100 == 0 || qi == 0)) {
            // Nota: printf in parallelo può mischiarsi, ma è accettabile per debug
            printf("          Query %d/%d\n", qi+1, input->nq);
        }
        ...
        // 7. Salva risultati (Thread safe: ogni thread scrive in un'area diversa)
